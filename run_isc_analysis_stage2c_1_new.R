# The sub-pipeline works on the Irish Shotgun Cohorts (ISC) data repository
...
load("C:\\Projects\\ELDERMET\\NatureAgingRevision\\ISC\\em_aging_data.RData")

species_group_shotgun <- species_groupings[order(species_groupings[,7]),]

G_Markers <- intersect(c("Bacteroides_fragilis","Actinomyces_odontolyticus","Bifidobacterium_dentium","Clostridium_clostridioforme","Clostridium_nexile","Clostridium_ramosum","Dialister_invisus","Eggerthella_lenta","Escherichia_coli","Fusobacterium_nucleatum","Granulicatella_adiacens","Lactococcus_lactis","Rothia_mucilaginosa","Ruminococcus_gnavus","Streptococcus_anginosus","Streptococcus_infantis","Streptococcus_salivarius","Streptococcus_sanguinis","Streptococcus_vestibularis","Subdoligranulum_variabile","Veillonella_atypica","Clostridium_asparagiforme","Clostridium_bolteae","Clostridium_citroniae","Clostridium_hathewayi","Clostridium_symbiosum","Streptococcus_parasanguinis","Solobacterium_moorei","Ruminococcus_torques","Streptococcus_mitis","Klebsiella_pneumoniae","Streptococcus_australis","Streptococcus_gordonii","Enterobacter_cloacae"),colnames(isc_select_age_final_species_clr))

L_Markers <- intersect(c("Eubacterium_hallii","Dorea_longicatena","Coprococcus_comes","Coprococcus_catus","Butyrivibrio_crossotus","Bacteroides_uniformis","Alistipes_shahii","Alistipes_indistinctus","Roseburia_hominis","Pseudoflavonifractor_capillosus","Eubacterium_siraeum","Eubacterium_rectale","Coprobacter_fastidiosus","Bifidobacterium_longum","Bifidobacterium_animalis","Barnesiella_intestinihominis","Bacteroides_xylanisolvens","Alistipes_senegalensis","Alistipes_putredinis","Alistipes_onderdonkii","Akkermansia_muciniphila","Faecalibacterium_prausnitzii","Roseburia_inulinivorans","Roseburia_intestinalis"),colnames(isc_select_age_final_species_clr))

df_isc_diversity_uniqueness$G_Markers <- rowMeans(apply(isc_select_age_final_species_clr[rownames(df_isc_diversity_uniqueness),G_Markers],2,range_scale1))

df_isc_diversity_uniqueness$L_Markers <- rowMeans(apply(isc_select_age_final_species_clr[rownames(df_isc_diversity_uniqueness),L_Markers],2,range_scale1))

df_isc_diversity_uniqueness$G1 <- rowMeans(apply(isc_select_age_final_species_clr[rownames(df_isc_diversity_uniqueness),intersect(rownames(species_group_shotgun[species_group_shotgun[,7]=="G1",]),colnames(isc_select_age_final_species_clr))],2,range_scale1))

df_isc_diversity_uniqueness$G2 <- rowMeans(apply(isc_select_age_final_species_clr[rownames(df_isc_diversity_uniqueness),intersect(rownames(species_group_shotgun[species_group_shotgun[,7]=="G2",]),colnames(isc_select_age_final_species_clr))],2,range_scale1))

G1Species <- intersect(rownames(species_groupings[species_groupings$BroadGroup=="G1",]),colnames(isc_select_age_final_species_clr))
G2Species <- intersect(rownames(species_groupings[species_groupings$BroadGroup=="G2",]),colnames(isc_select_age_final_species_clr))
OtherSpecies <- intersect(setdiff(rownames(species_groupings),c(G1Species,G2Species)),colnames(isc_select_age_final_species_clr))

df_isc_diversity_uniqueness <- as.data.frame(cbind(df_isc_diversity_uniqueness,isc_select_age_final_species_clr[rownames(df_isc_diversity_uniqueness),c(G1Species,G2Species,OtherSpecies)]))

df_em_aging_data[,"Charlson_Comorbidity"] <- (-1) * df_em_aging_data[,"Charlson_Comorbidity"]
df_em_aging_data[,"Geriatric_Depression"] <- (-1) * df_em_aging_data[,"Geriatric_Depression"]

df_em_diversity_uniqueness <- as.data.frame(cbind(df_isc_diversity_uniqueness[rownames(df_em_aging_data),],df_em_aging_data))

FinalIndexList <- c("InvFIM","InvBarthel","InvMMSE","Charlson_Comorbidity","Geriatric_Depression")
...
em_rlm_est_unhealthy_aging <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexList)))
rownames(em_rlm_est_unhealthy_aging) <- FinalFeatureList
colnames(em_rlm_est_unhealthy_aging) <- FinalIndexList
em_rlm_p_val_unhealthy_aging <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexList)))
rownames(em_rlm_p_val_unhealthy_aging) <- FinalFeatureList
colnames(em_rlm_p_val_unhealthy_aging) <- FinalIndexList
for(i in 1:length(FinalFeatureList))
{
	...
	for(j in 1:length(FinalIndexList))
	{
		...
		vec <- df_em_diversity_uniqueness[!is.na(df_em_diversity_uniqueness[,feature_name]),feature_name]
		if(length(vec[vec>0]) > 0)
		{
			temp_rlm <- rlm(as.formula(paste0(feature_name,"~",index_name)),data=df_em_diversity_uniqueness[!is.na(df_em_diversity_uniqueness[,index_name]),])
			summary_temp_rlm <- summary(temp_rlm)
			em_rlm_est_unhealthy_aging[feature_name,index_name] <- summary_temp_rlm$coefficients[2,3]
			em_rlm_p_val_unhealthy_aging[feature_name,index_name] <- f.robftest(temp_rlm)$p.value
		}
	}
}

em_rlm_q_val_unhealthy_aging <- apply(em_rlm_p_val_unhealthy_aging,2,function(x)(p.adjust(x,method="fdr")))

em_rlm_dir_unhealthy_aging <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexList)))
rownames(em_rlm_dir_unhealthy_aging) <- FinalFeatureList
colnames(em_rlm_dir_unhealthy_aging) <- FinalIndexList
for(i in 1:length(FinalFeatureList))
{
	for(j in 1:length(FinalIndexList))
	{
		em_rlm_dir_unhealthy_aging[i,j] <- ifelse(em_rlm_q_val_unhealthy_aging[i,j] <= 0.1,3*sign(em_rlm_est_unhealthy_aging[i,j]),ifelse(em_rlm_p_val_unhealthy_aging[i,j] <= 0.05,2*sign(em_rlm_est_unhealthy_aging[i,j]),sign(em_rlm_est_unhealthy_aging[i,j])))
	}
}

em_association_direction <- data.frame(decreased=apply(em_rlm_dir_unhealthy_aging,1,function(x)(length(x[x<= -2]))),increased=apply(em_rlm_dir_unhealthy_aging,1,function(x)(length(x[x>=2]))))

patient_list_young <- rownames(df_isc_diversity_uniqueness[(df_isc_diversity_uniqueness$study_name=="IBS")&(df_isc_diversity_uniqueness$study_condition=="IBS"),])
control_list_young <- rownames(df_isc_diversity_uniqueness[(df_isc_diversity_uniqueness$study_name=="IBS")&(df_isc_diversity_uniqueness$study_condition=="control"),])

FinalIndexListYoung <- c("IBS")

FinalFeatureList <- setdiff(c("species_shannon","species_bray_uniqueness","species_jaccard_uniqueness","species_aitchison_uniqueness","species_kendall_uniqueness","G_Markers","L_Markers","G1","G2",G1Species,G2Species,OtherSpecies),c("L_Markers_combined","G_Markers_combined"))

isc_rlm_est_unhealthy_young <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexListYoung)))
rownames(isc_rlm_est_unhealthy_young) <- FinalFeatureList
colnames(isc_rlm_est_unhealthy_young) <- FinalIndexListYoung
isc_rlm_p_val_unhealthy_young <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexListYoung)))
rownames(isc_rlm_p_val_unhealthy_young) <- FinalFeatureList
colnames(isc_rlm_p_val_unhealthy_young) <- FinalIndexListYoung
for(i in 1:length(FinalFeatureList))
{
	feature_name <- FinalFeatureList[i]
	print(feature_name)
	for(j in 1:length(FinalIndexListYoung))
	{
		index_name <- FinalIndexListYoung[j]
		vec_disease <- df_isc_diversity_uniqueness[patient_list_young,feature_name]
		vec_control <- df_isc_diversity_uniqueness[control_list_young,feature_name]
		vec <- c(vec_disease,vec_control)
		if(length(vec[vec>0]) > 0)
		{
			print("Index")
			temp_rlm <- rlm(df_isc_diversity_uniqueness[c(patient_list_young,control_list_young),feature_name]~as.factor(ifelse(df_isc_diversity_uniqueness[c(patient_list_young,control_list_young),"study_condition"]=="control",0,1)))
			summary_temp_rlm <- summary(temp_rlm)
			isc_rlm_est_unhealthy_young[feature_name,index_name] <- summary_temp_rlm$coefficients[2,3]
			isc_rlm_p_val_unhealthy_young[feature_name,index_name] <- f.robftest(temp_rlm)$p.value
		}
	}
}

isc_rlm_q_val_unhealthy_young <- apply(isc_rlm_p_val_unhealthy_young,2,function(x)(p.adjust(x,method="fdr")))

isc_rlm_dir_unhealthy_young <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexListYoung)))
rownames(isc_rlm_dir_unhealthy_young) <- FinalFeatureList
colnames(isc_rlm_dir_unhealthy_young) <- FinalIndexListYoung
for(i in 1:length(FinalFeatureList))
{
	for(j in 1:length(FinalIndexListYoung))
	{
		isc_rlm_dir_unhealthy_young[i,j] <- ifelse(isc_rlm_q_val_unhealthy_young[i,j] <= 0.1,3*sign(isc_rlm_est_unhealthy_young[i,j]),ifelse(isc_rlm_p_val_unhealthy_young[i,j] <= 0.05,2*sign(isc_rlm_est_unhealthy_young[i,j]),sign(isc_rlm_est_unhealthy_young[i,j])))
	}
}

isc_association_direction_young <- data.frame(decreased=apply(isc_rlm_dir_unhealthy_young,1,function(x)(length(x[x<= -2]))),increased=apply(isc_rlm_dir_unhealthy_young,1,function(x)(length(x[x>=2]))))

save.image("C:\\Projects\\ELDERMET\\NatureAgingRevision\\ISC\\isc_analysis_2021_Revision.RData")

save(em_rlm_est_unhealthy_aging,em_rlm_p_val_unhealthy_aging,em_rlm_q_val_unhealthy_aging,em_rlm_dir_unhealthy_aging,em_association_direction,df_em_diversity_uniqueness,isc_rlm_est_unhealthy_young,isc_rlm_p_val_unhealthy_young,isc_rlm_q_val_unhealthy_young,isc_rlm_dir_unhealthy_young,isc_association_direction_young,df_isc_diversity_uniqueness,file="C:\\Projects\\ELDERMET\\NatureAgingRevision\\em_stage_2c_results.RData")
