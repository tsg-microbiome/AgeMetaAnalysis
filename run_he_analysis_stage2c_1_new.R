...
G_Markers <- intersect(c("Bacteroides_fragilis","Actinomyces_odontolyticus","Bifidobacterium_dentium","Clostridium_clostridioforme","Clostridium_nexile","Clostridium_ramosum","Dialister_invisus","Eggerthella_lenta","Escherichia_coli","Fusobacterium_nucleatum","Granulicatella_adiacens","Lactococcus_lactis","Rothia_mucilaginosa","Ruminococcus_gnavus","Streptococcus_anginosus","Streptococcus_infantis","Streptococcus_salivarius","Streptococcus_sanguinis","Streptococcus_vestibularis","Subdoligranulum_variabile","Veillonella_atypica","Clostridium_asparagiforme","Clostridium_bolteae","Clostridium_citroniae","Clostridium_hathewayi","Clostridium_symbiosum","Streptococcus_parasanguinis","Solobacterium_moorei","Ruminococcus_torques","Streptococcus_mitis","Klebsiella_pneumoniae","Streptococcus_australis","Streptococcus_gordonii","Enterobacter_cloacae"),colnames(he_select_age_final_species_clr))

L_Markers <- intersect(c("Eubacterium_hallii","Dorea_longicatena","Coprococcus_comes","Coprococcus_catus","Butyrivibrio_crossotus","Bacteroides_uniformis","Alistipes_shahii","Alistipes_indistinctus","Roseburia_hominis","Pseudoflavonifractor_capillosus","Eubacterium_siraeum","Eubacterium_rectale","Coprobacter_fastidiosus","Bifidobacterium_longum","Bifidobacterium_animalis","Barnesiella_intestinihominis","Bacteroides_xylanisolvens","Alistipes_senegalensis","Alistipes_putredinis","Alistipes_onderdonkii","Akkermansia_muciniphila","Faecalibacterium_prausnitzii","Roseburia_inulinivorans","Roseburia_intestinalis"),colnames(he_select_age_final_species_clr))

df_he_diversity_uniqueness$G_Markers <- rowMeans(apply(he_select_age_final_species_clr[rownames(df_he_diversity_uniqueness),G_Markers],2,range_scale))

df_he_diversity_uniqueness$L_Markers <- rowMeans(apply(he_select_age_final_species_clr[rownames(df_he_diversity_uniqueness),L_Markers],2,range_scale))

df_he_diversity_uniqueness$G1 <- rowMeans(apply(he_select_age_final_species_clr[rownames(df_he_diversity_uniqueness),intersect(rownames(species_group_16S[species_group_16S[,7]=="G1",]),colnames(he_select_age_final_species_clr))],2,range_scale))

df_he_diversity_uniqueness$G2 <- rowMeans(apply(he_select_age_final_species_clr[rownames(df_he_diversity_uniqueness),intersect(rownames(species_group_16S[species_group_16S[,7]=="G2",]),colnames(he_select_age_final_species_clr))],2,range_scale))

G1Species <- intersect(rownames(species_groupings[species_groupings$BroadGroup=="G1",]),colnames(he_select_age_final_species_clr))
G2Species <- intersect(rownames(species_groupings[species_groupings$BroadGroup=="G2",]),colnames(he_select_age_final_species_clr))
OtherSpecies <- intersect(setdiff(rownames(species_groupings),c(G1Species,G2Species)),colnames(he_select_age_final_species_clr))

df_he_diversity_uniqueness <- as.data.frame(cbind(df_he_diversity_uniqueness,he_select_age_final_species_clr[rownames(df_he_diversity_uniqueness),c(G1Species,G2Species,OtherSpecies)]))

FinalIndexList <- c("Atherosclerosis","Cholecystitis","Colitis","Constipation","Diarrhea","Fatty_liver","Gastritis","IBS","Kidneystone","Rheumatoid_arthritis","Metabolic_syndrome","T2DM")
...
df_he_older_diversity_uniqueness <- df_he_diversity_uniqueness[df_he_diversity_uniqueness$age>=60,]

he_rlm_est_unhealthy_aging <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexList)))
rownames(he_rlm_est_unhealthy_aging) <- FinalFeatureList
colnames(he_rlm_est_unhealthy_aging) <- FinalIndexList
he_rlm_p_val_unhealthy_aging <- as.data.frame(matrix(1,length(FinalFeatureList),length(FinalIndexList)))
rownames(he_rlm_p_val_unhealthy_aging) <- FinalFeatureList
colnames(he_rlm_p_val_unhealthy_aging) <- FinalIndexList
for(i in 1:length(FinalFeatureList))
{
	...
	for(j in 1:length(FinalIndexList))
	{
		...
		control_list <- intersect(he_all_controls,rownames(df_he_older_diversity_uniqueness))
		diseased_list <- intersect(get(paste0("patient_list_",disease_name)),rownames(df_he_older_diversity_uniqueness))
		vec_control <- df_he_older_diversity_uniqueness[control_list,feature_name]
		vec_disease <- df_he_older_diversity_uniqueness[diseased_list,feature_name]
		total_vec <- c(vec_disease,vec_control)
		if(length(total_vec[total_vec > 0])>0)
		{
			temp_rlm <- rlm(df_he_older_diversity_uniqueness[c(diseased_list,control_list),feature_name]~as.factor(ifelse(c(diseased_list,control_list) %in% he_all_controls,0,1)))
			summary_temp_rlm <- summary(temp_rlm)
			he_rlm_est_unhealthy_aging[feature_name,disease_name] <- summary_temp_rlm$coefficients[2,3]
			#he_rlm_p_val_unhealthy_aging[feature_name,disease_name] <- f.robftest(temp_rlm)$p.value
			he_rlm_p_val_unhealthy_aging[feature_name,disease_name] <- wilcox.test(vec_control,vec_disease)$p.value
		}
	}
}

he_rlm_q_val_unhealthy_aging <- apply(he_rlm_p_val_unhealthy_aging,2,function(x)(p.adjust(x,method="fdr")))

he_rlm_dir_unhealthy_aging <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexList)))
rownames(he_rlm_dir_unhealthy_aging) <- FinalFeatureList
colnames(he_rlm_dir_unhealthy_aging) <- FinalIndexList
for(i in 1:length(FinalFeatureList))
{
	for(j in 1:length(FinalIndexList))
	{
		he_rlm_dir_unhealthy_aging[i,j] <- ifelse(he_rlm_q_val_unhealthy_aging[i,j] <= 0.1,3*sign(he_rlm_est_unhealthy_aging[i,j]),ifelse(he_rlm_p_val_unhealthy_aging[i,j] <= 0.05,2*sign(he_rlm_est_unhealthy_aging[i,j]),sign(he_rlm_est_unhealthy_aging[i,j])))
	}
}

he_association_direction <- data.frame(decreased=apply(he_rlm_dir_unhealthy_aging,1,function(x)(length(x[x<= -2]))),increased=apply(he_rlm_dir_unhealthy_aging,1,function(x)(length(x[x>=2]))))

df_he_younger_diversity_uniqueness <- df_he_diversity_uniqueness[df_he_diversity_uniqueness$age<60,]

he_rlm_est_unhealthy_young <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexList)))
rownames(he_rlm_est_unhealthy_young) <- FinalFeatureList
colnames(he_rlm_est_unhealthy_young) <- FinalIndexList
he_rlm_p_val_unhealthy_young <- as.data.frame(matrix(1,length(FinalFeatureList),length(FinalIndexList)))
rownames(he_rlm_p_val_unhealthy_young) <- FinalFeatureList
colnames(he_rlm_p_val_unhealthy_young) <- FinalIndexList
for(i in 1:length(FinalFeatureList))
{
	feature_name <- FinalFeatureList[i]
	print(feature_name)
	...
	{
		disease_name <- FinalIndexList[j]
		control_list <- intersect(he_all_controls,rownames(df_he_younger_diversity_uniqueness))
		diseased_list <- intersect(get(paste0("patient_list_",disease_name)),rownames(df_he_younger_diversity_uniqueness))
		vec_control <- df_he_younger_diversity_uniqueness[control_list,feature_name]
		vec_disease <- df_he_younger_diversity_uniqueness[diseased_list,feature_name]
		total_vec <- c(vec_disease,vec_control)
		if(length(total_vec[total_vec > 0])>0)
		{
			temp_rlm <- rlm(df_he_younger_diversity_uniqueness[c(diseased_list,control_list),feature_name]~as.factor(ifelse(c(diseased_list,control_list) %in% he_all_controls,0,1)))
			summary_temp_rlm <- summary(temp_rlm)
			he_rlm_est_unhealthy_young[feature_name,disease_name] <- summary_temp_rlm$coefficients[2,3]
			#he_rlm_p_val_unhealthy_aging[feature_name,disease_name] <- f.robftest(temp_rlm)$p.value
			he_rlm_p_val_unhealthy_young[feature_name,disease_name] <- wilcox.test(vec_control,vec_disease)$p.value
		}
	}
}

he_rlm_q_val_unhealthy_young <- apply(he_rlm_p_val_unhealthy_young,2,function(x)(p.adjust(x,method="fdr")))

he_rlm_dir_unhealthy_young <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexList)))
rownames(he_rlm_dir_unhealthy_young) <- FinalFeatureList
colnames(he_rlm_dir_unhealthy_young) <- FinalIndexList
for(i in 1:length(FinalFeatureList))
{
	for(j in 1:length(FinalIndexList))
	{
		he_rlm_dir_unhealthy_young[i,j] <- ifelse(he_rlm_q_val_unhealthy_young[i,j] <= 0.1,3*sign(he_rlm_est_unhealthy_young[i,j]),ifelse(he_rlm_p_val_unhealthy_young[i,j] <= 0.05,2*sign(he_rlm_est_unhealthy_young[i,j]),sign(he_rlm_est_unhealthy_young[i,j])))
	}
}

he_association_direction_young <- data.frame(decreased=apply(he_rlm_dir_unhealthy_young,1,function(x)(length(x[x<= -2]))),increased=apply(he_rlm_dir_unhealthy_young,1,function(x)(length(x[x>=2]))))


save(he_rlm_est_unhealthy_aging,he_rlm_p_val_unhealthy_aging,he_rlm_q_val_unhealthy_aging,he_rlm_dir_unhealthy_aging,he_association_direction,he_rlm_est_unhealthy_young,he_rlm_p_val_unhealthy_young,he_rlm_q_val_unhealthy_young,he_rlm_dir_unhealthy_young,he_association_direction_young,df_he_diversity_uniqueness,he_all_controls,file="C:\\Projects\\ELDERMET\\NatureAgingRevision\\he_stage_2c_results.RData")
