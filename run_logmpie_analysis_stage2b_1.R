...
logmpie_all_controls <- rownames(df_logmpie_diversity_uniqueness)

HighlyDetectedSpecies <- names(which(100*colSums(apply(logmpie_select_age_final_species,2,function(x)(ifelse(x>0,1,0))))/nrow(logmpie_select_age_final_species)>=5))
...
G_Markers <- intersect(c("Bacteroides_fragilis","Actinomyces_odontolyticus","Bifidobacterium_dentium","Clostridium_clostridioforme","Clostridium_nexile","Clostridium_ramosum","Dialister_invisus","Eggerthella_lenta","Escherichia_coli","Fusobacterium_nucleatum","Granulicatella_adiacens","Lactococcus_lactis","Rothia_mucilaginosa","Ruminococcus_gnavus","Streptococcus_anginosus","Streptococcus_infantis","Streptococcus_salivarius","Streptococcus_sanguinis","Streptococcus_vestibularis","Subdoligranulum_variabile","Veillonella_atypica","Clostridium_asparagiforme","Clostridium_bolteae","Clostridium_citroniae","Clostridium_hathewayi","Clostridium_symbiosum","Streptococcus_parasanguinis","Solobacterium_moorei","Ruminococcus_torques","Streptococcus_mitis","Klebsiella_pneumoniae","Streptococcus_australis","Streptococcus_gordonii","Enterobacter_cloacae"),colnames(logmpie_combined_df_sum_stat_species))

L_Markers <- intersect(c("Eubacterium_hallii","Dorea_longicatena","Coprococcus_comes","Coprococcus_catus","Butyrivibrio_crossotus","Bacteroides_uniformis","Alistipes_shahii","Alistipes_indistinctus","Roseburia_hominis","Pseudoflavonifractor_capillosus","Eubacterium_siraeum","Eubacterium_rectale","Coprobacter_fastidiosus","Bifidobacterium_longum","Bifidobacterium_animalis","Barnesiella_intestinihominis","Bacteroides_xylanisolvens","Alistipes_senegalensis","Alistipes_putredinis","Alistipes_onderdonkii","Akkermansia_muciniphila","Faecalibacterium_prausnitzii","Roseburia_inulinivorans","Roseburia_intestinalis"),colnames(logmpie_combined_df_sum_stat_species))

logmpie_combined_df_sum_stat_species$G_Markers_combined <- rowMeans(apply(logmpie_combined_df_sum_stat_species[,G_Markers],2,rank_scale))
logmpie_combined_df_sum_stat_species$L_Markers_combined <- rowMeans(apply(logmpie_combined_df_sum_stat_species[,L_Markers],2,rank_scale))
...
save(df_logmpie_diversity_uniqueness,df_logmpie_controls_diversity_uniqueness,logmpie_select_age_final_species,logmpie_combined_df_sum_stat_species,file="C:\\Projects\\ELDERMET\\NatureAgingRevision\\logmpie_stage2b_results.RData") 
