# The sub-pipeline works on the American Gut data repository
...
# diseases in the American Gut dataset
...
ag_elderly <- rownames(df_ag_diversity_uniqueness[df_ag_diversity_uniqueness$age>=60,])
ag_young <- rownames(df_ag_diversity_uniqueness[df_ag_diversity_uniqueness$age<60,])
...
ag_all_controls <- names(which(apply(ag_select_age_final_metadata[,disease_list],1,function(x)(length(x[(x=="Yes")])))==0))

species_group_16S <- species_groupings[order(species_groupings[,7]),]

G_Markers <- intersect(c("Bacteroides_fragilis","Actinomyces_odontolyticus","Bifidobacterium_dentium","Clostridium_clostridioforme","Clostridium_nexile","Clostridium_ramosum","Dialister_invisus","Eggerthella_lenta","Escherichia_coli","Fusobacterium_nucleatum","Granulicatella_adiacens","Lactococcus_lactis","Rothia_mucilaginosa","Ruminococcus_gnavus","Streptococcus_anginosus","Streptococcus_infantis","Streptococcus_salivarius","Streptococcus_sanguinis","Streptococcus_vestibularis","Subdoligranulum_variabile","Veillonella_atypica","Clostridium_asparagiforme","Clostridium_bolteae","Clostridium_citroniae","Clostridium_hathewayi","Clostridium_symbiosum","Streptococcus_parasanguinis","Solobacterium_moorei","Ruminococcus_torques","Streptococcus_mitis","Klebsiella_pneumoniae","Streptococcus_australis","Streptococcus_gordonii","Enterobacter_cloacae"),colnames(ag_select_age_final_species_clr))

L_Markers <- intersect(c("Eubacterium_hallii","Dorea_longicatena","Coprococcus_comes","Coprococcus_catus","Butyrivibrio_crossotus","Bacteroides_uniformis","Alistipes_shahii","Alistipes_indistinctus","Roseburia_hominis","Pseudoflavonifractor_capillosus","Eubacterium_siraeum","Eubacterium_rectale","Coprobacter_fastidiosus","Bifidobacterium_longum","Bifidobacterium_animalis","Barnesiella_intestinihominis","Bacteroides_xylanisolvens","Alistipes_senegalensis","Alistipes_putredinis","Alistipes_onderdonkii","Akkermansia_muciniphila","Faecalibacterium_prausnitzii","Roseburia_inulinivorans","Roseburia_intestinalis"),colnames(ag_select_age_final_species_clr))

df_ag_diversity_uniqueness$G_Markers <- apply(apply(ag_select_age_final_species_clr[rownames(df_ag_diversity_uniqueness),G_Markers],2,range_scale),1,function(x)(mean(x[!is.nan(x)])))

df_ag_diversity_uniqueness$L_Markers <- apply(apply(ag_select_age_final_species_clr[rownames(df_ag_diversity_uniqueness),L_Markers],2,range_scale),1,function(x)(mean(x[!is.nan(x)])))

df_ag_diversity_uniqueness$G1 <- rowMeans(apply(ag_select_age_final_species_clr[rownames(df_ag_diversity_uniqueness),intersect(rownames(species_group_16S[species_group_16S[,7]=="G1",]),colnames(ag_select_age_final_species_clr))],2,range_scale))

df_ag_diversity_uniqueness$G2 <- rowMeans(apply(ag_select_age_final_species_clr[rownames(df_ag_diversity_uniqueness),intersect(rownames(species_group_16S[species_group_16S[,7]=="G2",]),colnames(ag_select_age_final_species_clr))],2,range_scale))

df_ag_diversity_uniqueness$Other <- rowMeans(apply(apply(ag_select_disease_final_species_clr[rownames(df_ag_diversity_uniqueness),intersect(colnames(ag_select_disease_final_species_clr),rownames(species_groupings[(species_groupings$BroadGroup!="G1")&(species_groupings$BroadGroup!="G2"),]))],2,range_scale),2,function(x)(ifelse(is.nan(x),0,x))))

G1Species <- intersect(rownames(species_groupings[species_groupings$BroadGroup=="G1",]),colnames(ag_select_age_final_species_clr))
G2Species <- intersect(rownames(species_groupings[species_groupings$BroadGroup=="G2",]),colnames(ag_select_age_final_species_clr))
OtherSpecies <- intersect(setdiff(rownames(species_groupings),c(G1Species,G2Species)),colnames(ag_select_age_final_species_clr))

df_ag_diversity_uniqueness <- as.data.frame(cbind(df_ag_diversity_uniqueness,ag_select_age_final_species_clr[rownames(df_ag_diversity_uniqueness),c(G1Species,G2Species,OtherSpecies)]))
...
FinalIndexList <- c("alzheimers","asd","cancer","cardiovascular_disease","cdiff","diabetes","ibd","ibs","kidney_disease","liver_disease","lung_disease","migraine","sibo")
...
ag_rlm_est_unhealthy_aging <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexList)))
rownames(ag_rlm_est_unhealthy_aging) <- FinalFeatureList
colnames(ag_rlm_est_unhealthy_aging) <- FinalIndexList
ag_rlm_p_val_unhealthy_aging <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexList)))
rownames(ag_rlm_p_val_unhealthy_aging) <- FinalFeatureList
colnames(ag_rlm_p_val_unhealthy_aging) <- FinalIndexList
...
for(i in 1:length(FinalFeatureList))
{
	...
	for(j in 1:length(FinalIndexList))
	{
		...
		control_list <- intersect(ag_elderly,ag_all_controls)
		diseased_list <- intersect(ag_elderly,get(paste0("patient_list_",disease_name)))
		vec_control <- df_ag_diversity_uniqueness[control_list,feature_name]
		vec_disease <- df_ag_diversity_uniqueness[diseased_list,feature_name]
		total_vec <- c(vec_disease,vec_control)
		if(length(total_vec[total_vec > 0])>0)
		{
			temp_rlm <- rlm(df_ag_diversity_uniqueness[c(diseased_list,control_list),feature_name]~as.factor(ifelse(c(diseased_list,control_list) %in% ag_all_controls,0,1)))
			summary_temp_rlm <- summary(temp_rlm)
			ag_rlm_est_unhealthy_aging[feature_name,disease_name] <- summary_temp_rlm$coefficients[2,3]
			ag_rlm_p_val_unhealthy_aging[feature_name,disease_name] <- wilcox.test(vec_disease,vec_control)$p.value
		}
	}
}

ag_rlm_q_val_unhealthy_aging <- apply(ag_rlm_p_val_unhealthy_aging,2,function(x)(p.adjust(x,method="fdr")))

ag_rlm_dir_unhealthy_aging <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexList)))
rownames(ag_rlm_dir_unhealthy_aging) <- FinalFeatureList
colnames(ag_rlm_dir_unhealthy_aging) <- FinalIndexList
for(i in 1:length(FinalFeatureList))
{
	for(j in 1:length(FinalIndexList))
	{
		ag_rlm_dir_unhealthy_aging[i,j] <- ifelse(ag_rlm_q_val_unhealthy_aging[i,j] <= 0.1,3*sign(ag_rlm_est_unhealthy_aging[i,j]),ifelse(ag_rlm_p_val_unhealthy_aging[i,j] <= 0.05,2*sign(ag_rlm_est_unhealthy_aging[i,j]),sign(ag_rlm_est_unhealthy_aging[i,j])))
	}
}

ag_association_direction <- data.frame(decreased=apply(ag_rlm_dir_unhealthy_aging,1,function(x)(length(x[x<= -2]))),increased=apply(ag_rlm_dir_unhealthy_aging,1,function(x)(length(x[x>=2]))))

ag_rlm_est_unhealthy_young <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexList)))
rownames(ag_rlm_est_unhealthy_young) <- FinalFeatureList
colnames(ag_rlm_est_unhealthy_young) <- FinalIndexList
ag_rlm_p_val_unhealthy_young <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexList)))
rownames(ag_rlm_p_val_unhealthy_young) <- FinalFeatureList
colnames(ag_rlm_p_val_unhealthy_young) <- FinalIndexList
...
for(i in 1:length(FinalFeatureList))
{
	...
	{
		disease_name <- FinalIndexList[j]
		control_list <- intersect(ag_young,ag_all_controls)
		diseased_list <- intersect(ag_young,get(paste0("patient_list_",disease_name)))
		vec_control <- df_ag_diversity_uniqueness[control_list,feature_name]
		vec_disease <- df_ag_diversity_uniqueness[diseased_list,feature_name]
		total_vec <- c(vec_disease,vec_control)
		if(length(total_vec[total_vec > 0])>0)
		{
			temp_rlm <- rlm(df_ag_diversity_uniqueness[c(diseased_list,control_list),feature_name]~as.factor(ifelse(c(diseased_list,control_list) %in% ag_all_controls,0,1)))
			summary_temp_rlm <- summary(temp_rlm)
			ag_rlm_est_unhealthy_young[feature_name,disease_name] <- summary_temp_rlm$coefficients[2,3]
			ag_rlm_p_val_unhealthy_young[feature_name,disease_name] <- wilcox.test(vec_disease,vec_control)$p.value
		}
	}
}

ag_rlm_q_val_unhealthy_young <- apply(ag_rlm_p_val_unhealthy_aging,2,function(x)(p.adjust(x,method="fdr")))

ag_rlm_dir_unhealthy_young <- as.data.frame(matrix(0,length(FinalFeatureList),length(FinalIndexList)))
rownames(ag_rlm_dir_unhealthy_young) <- FinalFeatureList
colnames(ag_rlm_dir_unhealthy_young) <- FinalIndexList
for(i in 1:length(FinalFeatureList))
{
	for(j in 1:length(FinalIndexList))
	{
		ag_rlm_dir_unhealthy_young[i,j] <- ifelse(ag_rlm_q_val_unhealthy_young[i,j] <= 0.1,3*sign(ag_rlm_est_unhealthy_young[i,j]),ifelse(ag_rlm_p_val_unhealthy_young[i,j] <= 0.05,2*sign(ag_rlm_est_unhealthy_young[i,j]),sign(ag_rlm_est_unhealthy_young[i,j])))
	}
}

ag_association_direction_young <- data.frame(decreased=apply(ag_rlm_dir_unhealthy_young,1,function(x)(length(x[x<= -2]))),increased=apply(ag_rlm_dir_unhealthy_young,1,function(x)(length(x[x>=2]))))


save(ag_rlm_est_unhealthy_aging,ag_rlm_p_val_unhealthy_aging,ag_rlm_q_val_unhealthy_aging,ag_rlm_dir_unhealthy_aging,ag_association_direction,ag_rlm_est_unhealthy_young,ag_rlm_p_val_unhealthy_young,ag_rlm_q_val_unhealthy_young,ag_rlm_dir_unhealthy_young,ag_association_direction_young,df_ag_diversity_uniqueness,ag_all_controls,file="C:\\Projects\\ELDERMET\\NatureAgingRevision\\ag_stage_2c_results.RData")
