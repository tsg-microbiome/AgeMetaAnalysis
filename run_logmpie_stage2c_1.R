...
load("C:\\Projects\\ELDERMET\\NatureAgingRevision\\Odamaki\\logmpie_analysis_2021_Revision.RData")
load("C:\\Projects\\ELDERMET\\NatureAgingRevision\\taxa_groupings.RData")

species_group_16S <- species_group_16S[order(species_group_shotgun[,7]),]

G_Markers <- intersect(c("Bacteroides_fragilis","Actinomyces_odontolyticus","Bifidobacterium_dentium","Clostridium_clostridioforme","Clostridium_nexile","Clostridium_ramosum","Dialister_invisus","Eggerthella_lenta","Escherichia_coli","Fusobacterium_nucleatum","Granulicatella_adiacens","Lactococcus_lactis","Rothia_mucilaginosa","Ruminococcus_gnavus","Streptococcus_anginosus","Streptococcus_infantis","Streptococcus_salivarius","Streptococcus_sanguinis","Streptococcus_vestibularis","Subdoligranulum_variabile","Veillonella_atypica","Clostridium_asparagiforme","Clostridium_bolteae","Clostridium_citroniae","Clostridium_hathewayi","Clostridium_symbiosum","Streptococcus_parasanguinis","Solobacterium_moorei","Ruminococcus_torques","Streptococcus_mitis","Klebsiella_pneumoniae","Streptococcus_australis","Streptococcus_gordonii","Enterobacter_cloacae"),colnames(logmpie_select_age_final_species))

L_Markers <- intersect(c("Eubacterium_hallii","Dorea_longicatena","Coprococcus_comes","Coprococcus_catus","Butyrivibrio_crossotus","Bacteroides_uniformis","Alistipes_shahii","Alistipes_indistinctus","Roseburia_hominis","Pseudoflavonifractor_capillosus","Eubacterium_siraeum","Eubacterium_rectale","Coprobacter_fastidiosus","Bifidobacterium_longum","Bifidobacterium_animalis","Barnesiella_intestinihominis","Bacteroides_xylanisolvens","Alistipes_senegalensis","Alistipes_putredinis","Alistipes_onderdonkii","Akkermansia_muciniphila","Faecalibacterium_prausnitzii","Roseburia_inulinivorans","Roseburia_intestinalis"),colnames(logmpie_select_age_final_species))

df_logmpie_diversity_uniqueness$G_Markers <- rowMeans(apply(logmpie_select_age_final_species[rownames(df_logmpie_diversity_uniqueness),G_Markers],2,rank_scale))

df_logmpie_diversity_uniqueness$L_Markers <- rowMeans(apply(logmpie_select_age_final_species[rownames(df_logmpie_diversity_uniqueness),L_Markers],2,rank_scale))

df_logmpie_diversity_uniqueness$SpeciesGroup1 <- rowMeans(apply(logmpie_select_age_final_species[rownames(df_logmpie_diversity_uniqueness),intersect(rownames(species_group_shotgun[species_group_shotgun[,7]=="G1",]),colnames(logmpie_select_age_final_species))],2,rank_scale))

df_logmpie_diversity_uniqueness$SpeciesGroup2 <- rowMeans(apply(logmpie_select_age_final_species[rownames(df_logmpie_diversity_uniqueness),intersect(rownames(species_group_shotgun[species_group_shotgun[,7]=="G2",]),colnames(logmpie_select_age_final_species))],2,rank_scale))

df_logmpie_diversity_uniqueness$SpeciesGroup3 <- rowMeans(apply(logmpie_select_age_final_species[rownames(df_logmpie_diversity_uniqueness),intersect(rownames(species_group_shotgun[species_group_shotgun[,7]=="G3",]),colnames(logmpie_select_age_final_species))],2,rank_scale))

df_logmpie_diversity_uniqueness$SpeciesGroup4 <- rowMeans(apply(logmpie_select_age_final_species[rownames(df_logmpie_diversity_uniqueness),intersect(rownames(species_group_shotgun[species_group_shotgun[,7]=="G4",]),colnames(logmpie_select_age_final_species))],2,rank_scale))

save(df_logmpie_diversity_uniqueness,file="C:\\Projects\\ELDERMET\\NatureAgingRevision\\logmpie_stage_2c_results.RData")